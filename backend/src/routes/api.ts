...
// Кэш курсов НБРБ (Национальный банк Республики Беларусь)
let cachedUsdRate = 3.27;
let cachedEurRate = 3.52;
let lastRatesFetch = 0;

// Живой запрос курсов с официального API НБРБ
async function fetchOfficialRates(): Promise<void> {
    const now = Date.now();
    // Обновляем курс не чаще чем раз в час
    if (now - lastRatesFetch < 3600000) return;

    try {
        const usdRes = await fetch('https://nbrb.by/api/exrates/rates/USD?parammode=2');
        if (usdRes.ok) {
            const usdData = await usdRes.json() as { Cur_OfficialRate: number };
            cachedUsdRate = usdData.Cur_OfficialRate;
        }

        const eurRes = await fetch('https://nbrb.by/api/exrates/rates/EUR?parammode=2');
        if (eurRes.ok) {
            const eurData = await eurRes.json() as { Cur_OfficialRate: number };
            cachedEurRate = eurData.Cur_OfficialRate;
        }

        lastRatesFetch = now;
        console.log(`💱 Актуальные курсы НБРБ обновлены: 1 USD = ${cachedUsdRate} BYN, 1 EUR = ${cachedEurRate} BYN`);
    } catch (err) {
        console.error('⚠️ Не удалось загрузить курсы НБРБ, используем кэш:', err);
    }
}
...
// Расчет свободных денег по ЖИВОМУ курсу Нацбанка РБ
router.get('/analytics/free-cash', async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        accounts.forEach(acc => {
            if (acc.currency_code === 'BYN') totalInByn += acc.balance;
            else if (acc.currency_code === 'USD' || acc.currency_code === 'USDT') totalInByn += acc.balance * cachedUsdRate;
            else if (acc.currency_code === 'EUR') totalInByn += acc.balance * cachedEurRate;
        });
        ...
        res.json({
            ...
            usd_rate: cachedUsdRate.toFixed(4)
        });
    } catch (err: any) {
